...
data class Person2(val name: String, val age: Int, val team: String)

class Part3Tests {

    @Test
    fun testJsonReflection() {
        val person = Person2("Pizzi", 34, "Estoril Praia")
        val result = person.membersToJson()
    ...
    data class Team(val players: List<Person2>)

    @Test
    fun testIterableClass() {
        val team = Team(listOf(
            Person2("Pizzi", 34, "Benfica"),
            Person2("Cristiano Ronaldo", 41, "Al-Nassr")
        ))

        val result = team.membersToJson()
